...
def to_smiles(structures,mw,clogp,TPSA,rb,hba,hbd):
    smi_out = open('test.csv', 'w')
    print("Smiles,IDNUMBER, MWt, cLogP, TPSA, RotB, HBA, HBD", file=smi_out)
    print(len(structures), len(ids))
    for i in range(len(structures)):
        print('{0},{1},{2:0.3f},{3:0.3f},{4:0.2f},{5},{6},{7}'.format(Chem.MolToSmiles(structures[ i ]), 
                   ids[ i ],mw[i],clogp[i], TPSA[i], rb[i], hbd[i],hba[i]), sep=',', file=smi_out)

def plot_properties(mol,mw, clogp,TPSA,rb,hba,hbd): 
...
#    tpsa = df['TPSA'].values.tolist()
    ...
    mol = df['ROMol']
    MWs = [Descriptors.ExactMolWt(structure) for structure in mol]
    HBAs = [Descriptors.NumHAcceptors(structure) for structure in mol]
    HBDs = [Descriptors.NumHDonors(structure) for structure in mol]
    cLogPs = [Descriptors.MolLogP(structure) for structure in mol]
    tpsa = [Descriptors.TPSA(structure) for structure in mol]
    rotb = [Descriptors.NumRotatableBonds(structure) for structure in mol]
    for structure in mol:
        id = structure.GetProp('_Name')
        ids.append(id)
    to_smiles(mol,MWs,cLogPs,tpsa,rotb,HBAs,HBDs)
    plot_properties(mol,MWs,cLogPs,tpsa,rotb,HBAs,HBDs)  
    #substr_search(mol)
